"""Credential the adjudicator resolving the current revocation-checkpoint conflict."""
...
CREDENTIAL_PREFIX = (
    "current_revocation_checkpoint_witness_conflict_adjudicator_credential"
)
...
class CredentialBoundCurrentRevocationCheckpointWitnessConflictCorpusSnapshot:
    """Exact `1.24.0` adjudication plus issuer-bound credential evidence."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise CredentialError(
                "credential predecessor differs from exact 1.24.0 corpus"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise CredentialError("credential content order differs from 1.24.0")
        ...
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise CredentialError("credential successor may not precede 1.24.0")
    ...
    def from_document(
        ...
        *,
        predecessor: AdjudicationBoundCurrentRevocationCheckpointWitnessCorpusSnapshot,
    ) -> Self:
...
def load_current_revocation_checkpoint_witness_conflict_credential_evidence(
    ...
    *,
    ...
    """Load and reverify the complete `1.25.0` credential graph."""
...
def validate_current_revocation_checkpoint_witness_conflict_credentials(
    *,
    plan: ExperimentPlan,
    corpus: CredentialBoundCurrentRevocationCheckpointWitnessConflictCorpusSnapshot,
    adjudicator_registry: WitnessConflictAdjudicatorRegistrySnapshot,
    ...
    """Validate exact authorization without altering conflict adjudication."""
...
def persist_current_revocation_checkpoint_witness_conflict_credential_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: CredentialBoundCurrentRevocationCheckpointWitnessConflictCorpusSnapshot,
    predecessor_corpus: AdjudicationBoundCurrentRevocationCheckpointWitnessCorpusSnapshot,
    adjudicator_registry: WitnessConflictAdjudicatorRegistrySnapshot,
    issuer_registry: CredentialIssuerRegistrySnapshot,
    credential_policy: CredentialPolicySnapshot,
    attestations: tuple[CredentialAttestationSnapshot, ...],
    ...
    """Publish issuer, policy, credential, then the `1.25.0` manifest last."""
