"""Checkpoint the exact witness-conflict adjudicator credential revocation ledger head."""
...
class CheckpointBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCorpusSnapshot:
    """Compact successor binding the exact 1.11.0 ledger checkpoint head."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise AdjudicatorCredentialRevocationCheckpointError(
                "witness-conflict revocation checkpoint corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCredentialRevocationCheckpointError(
                "witness-conflict checkpoint predecessor differs from corpus"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "witness-conflict checkpoint content order differs from predecessor"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "witness-conflict checkpoint content IDs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint successor may not precede its 1.11.0 predecessor"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "witness-conflict checkpoint corpus hash differs from payload"
            )
    ...
    def from_document(
        ...
        *,
        ...
        if unknown:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "witness-conflict checkpoint corpus contains unsupported fields: "
                + ", ".join(unknown)
            )
        content_ids = document.get("content_ids")
        if not isinstance(content_ids, list):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "witness-conflict checkpoint content_ids must be an array"
            )
        ...
        return cls(
            ...
            checkpoint_log_ref=_versioned_ref(
                document.get(f"{prefix}_log_ref"),
                f"{prefix}_log_ref",
            ),
            checkpoint_head_ref=StoredArtifactRef.from_document(
                _mapping(
                    document.get(f"{prefix}_head_ref"),
                    f"{prefix}_head_ref",
                )
            ),
...
def load_checkpoint_conflict_witness_adjudicator_credential_revocation_checkpoint_evidence(
    ...
    *,
    ...
    """Load the generic checkpoint graph through the context manifest."""
...
def validate_checkpoint_conflict_witness_adjudicator_credential_revocation_checkpoints(
    *,
    plan: ExperimentPlan,
    corpus: CheckpointBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCorpusSnapshot,
    policy: AdjudicatorCredentialRevocationCheckpointPolicySnapshot,
    ...
    """Verify exact ledger coverage before the delegated revocation decision."""
...
def persist_checkpoint_bound_checkpoint_conflict_witness_adjudicator_credential_revocation_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: CheckpointBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCorpusSnapshot,
    predecessor_corpus: RevocationBoundCheckpointConflictWitnessAdjudicatorCredentialCorpusSnapshot,
    policy: AdjudicatorCredentialRevocationCheckpointPolicySnapshot,
    log: AdjudicatorCredentialRevocationCheckpointLogSnapshot,
    ledger: AdjudicatorCredentialRevocationLedgerSnapshot,
    checkpoints: tuple[AdjudicatorCredentialRevocationLedgerCheckpointSnapshot, ...],
    ...
    """Append policy, checkpoints, log, then publish the 1.12.0 manifest last."""
