...
class WitnessBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCheckpointCorpusSnapshot:
    """Compact successor binding named witnesses to the immutable 1.12.0 head."""

    corpus: CheckpointBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCorpusSnapshot
    corpus_id: str
    corpus_version: str
    status: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness predecessor differs from corpus"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness content order differs from predecessor"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness content IDs must be unique"
            )
        if not self.witness_attestation_refs:
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness corpus requires attestations"
            )
        if len(self.witness_attestation_refs) != len(set(self.witness_attestation_refs)):
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness attestation refs must be unique"
            )
        ...
        if self.artifact_hash != expected:
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness corpus hash differs from payload"
            )
    ...
    def from_document(
        ...
        *,
        predecessor: CheckpointBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCorpusSnapshot,
    ) -> WitnessBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCheckpointCorpusSnapshot:
        prefix = (
        ...
        if unknown:
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness corpus contains unsupported fields: "
                + ", ".join(unknown)
            )
        content_ids = document.get("content_ids")
        if not isinstance(content_ids, list):
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness content_ids must be an array"
            )
        attestation_refs = document.get(f"{prefix}_attestation_refs")
        if not isinstance(attestation_refs, list):
            raise AdjudicatorCheckpointWitnessError(
                "witness-conflict adjudicator witness attestation refs must be an array"
            )
        ...
        return cls(
            ...
            witness_attestation_refs=tuple(
                StoredArtifactRef.from_document(
                    _mapping(item, "witness-conflict adjudicator witness attestation ref")
                )
...
def load_checkpoint_conflict_witness_adjudicator_credential_revocation_checkpoint_witness_evidence(
    store: FileSystemArtifactStore,
    *,
    corpus: WitnessBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCheckpointCorpusSnapshot,
...
def validate_checkpoint_conflict_witness_adjudicator_credential_revocation_checkpoint_witness_attestations(
    *,
    plan: ExperimentPlan,
    corpus: WitnessBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCheckpointCorpusSnapshot,
    registry: CheckpointWitnessRegistrySnapshot,
...
def persist_witness_bound_checkpoint_conflict_witness_adjudicator_credential_revocation_checkpoint_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: WitnessBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCheckpointCorpusSnapshot,
    predecessor_corpus: CheckpointBoundCheckpointConflictWitnessAdjudicatorCredentialRevocationCorpusSnapshot,
    registry: CheckpointWitnessRegistrySnapshot,
    policy: CheckpointWitnessPolicySnapshot,
    head_checkpoint: AdjudicatorCredentialRevocationLedgerCheckpointSnapshot,
    attestations: tuple[CheckpointWitnessAttestationSnapshot, ...],
