"""Adjudicate conflicts over the exact `1.28.0` named-witness population."""
...
class ConflictingCurrentRevocationConflictAdjudicatorCheckpointWitnessCorpusSnapshot:
    """The exact conflicting witness population over the `1.27.0` head."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise ConflictAdjudicationError(
                "current conflict-adjudication corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise ConflictAdjudicationError(
                "conflicting witness predecessor differs from exact 1.27.0"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise ConflictAdjudicationError(
                "conflicting witness content order differs from 1.27.0"
            )
        if len(self.declared_content_ids) != len(
            set(self.declared_content_ids)
        ):
            raise ConflictAdjudicationError(
                "conflicting witness content IDs must be unique"
            )
        if not self.witness_attestation_refs:
            raise ConflictAdjudicationError(
                "conflict adjudication requires witness attestations"
            )
        if len(self.witness_attestation_refs) != len(
            set(self.witness_attestation_refs)
        ):
            raise ConflictAdjudicationError(
                "conflicting witness attestation refs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise ConflictAdjudicationError(
                "conflict adjudication may not precede 1.27.0"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise ConflictAdjudicationError(
                "conflict-adjudication corpus hash differs from payload"
            )
    ...
    def from_document(
        ...
        *,
        ...
        if unknown:
            raise ConflictAdjudicationError(
                "conflict-adjudication corpus contains unsupported fields: "
                + ", ".join(unknown)
            )
        ...
        if not isinstance(refs, list):
            raise ConflictAdjudicationError(
                "conflicting witness attestation refs must be an array"
            )
        ...
        return cls(
            ...
            witness_attestation_refs=tuple(
                StoredArtifactRef.from_document(
                    _mapping(item, "conflicting witness attestation ref")
                )
                for item in refs
            ),
...
@dataclass(frozen=True, slots=True)
class AdjudicationBoundCurrentRevocationConflictAdjudicatorCheckpointWitnessCorpusSnapshot:
    """The conflicting population plus exact adjudication authority."""

    corpus: ConflictingCurrentRevocationConflictAdjudicatorCheckpointWitnessCorpusSnapshot
    ...
    def __post_init__(self) -> None:
        if self.predecessor_corpus_ref != self.witness_predecessor.reference():
            raise ConflictAdjudicationError(
                "adjudication predecessor differs from exact 1.28.0 corpus"
            )
        if self.content_ids != self.witness_predecessor.content_ids:
            raise ConflictAdjudicationError(
                "adjudication content order differs from 1.28.0"
            )
        if (
            ...
            raise ConflictAdjudicationError(
                "adjudication changed the 1.27.0 checkpoint predecessor"
            )
        if (
        ...
        if _parse_timestamp(
            ...
            raise ConflictAdjudicationError(
                "adjudication successor may not precede 1.28.0"
            )
    ...
    def from_document(
        ...
        *,
        ...
        conflicting = (
            ConflictingCurrentRevocationConflictAdjudicatorCheckpointWitnessCorpusSnapshot.from_document(
                document,
...
def load_current_revocation_conflict_adjudicator_checkpoint_adjudication_evidence(
    store: FileSystemArtifactStore,
    *,
    corpus: AdjudicationBoundCurrentRevocationConflictAdjudicatorCheckpointWitnessCorpusSnapshot,
    ...
    """Load and reverify the complete `1.29.0` adjudication graph."""
    ...
    if corpus_artifact.payload != corpus.artifact().payload:
        raise ArtifactIntegrityError(
            "stored current conflict-adjudication corpus differs"
        )
    ...
    if registry_artifact.payload != adjudicator_registry.canonical_payload:
        raise ArtifactIntegrityError(
            "stored current conflict-adjudicator registry differs"
        )
    ...
    if policy_artifact.payload != adjudication_policy.canonical_payload:
        raise ArtifactIntegrityError(
            "stored current conflict-adjudication policy differs"
        )
    ...
    if stored_adjudication.reference() != corpus.adjudication_ref:
        raise ArtifactIntegrityError(
            "stored current conflict adjudication differs from corpus"
        )
    ...
    return StoredConflictAdjudicationEvidence(
        ...
        adjudication_policy_ref=store.reference(
            adjudication_policy.policy_id
        ),
...
def validate_current_revocation_conflict_adjudicator_checkpoint_adjudication(
    *,
    plan: ExperimentPlan,
    corpus: AdjudicationBoundCurrentRevocationConflictAdjudicatorCheckpointWitnessCorpusSnapshot,
    witness_registry: CheckpointWitnessRegistrySnapshot,
...
def persist_current_revocation_conflict_adjudicator_checkpoint_adjudication_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: AdjudicationBoundCurrentRevocationConflictAdjudicatorCheckpointWitnessCorpusSnapshot,
    ...
    """Append dependencies and publish the `1.29.0` manifest last."""
    ...
    if predecessor.payload != witness_predecessor.artifact().payload:
        raise ArtifactIntegrityError(
            "stored 1.28.0 witness predecessor differs"
        )
    ...
    for artifact in (
        ...
        *(item.artifact() for item in witness_attestations),
        ...
        if stored.artifact_hash != artifact.artifact_hash:
            raise ArtifactIntegrityError(
                "stored current conflict graph differs"
            )
    ...
    return load_current_revocation_conflict_adjudicator_checkpoint_adjudication_evidence(
        store,
        corpus=corpus,
        witness_registry=witness_registry,
        witness_policy=witness_policy,
