"""Adjudicate conflicts over the exact current checkpoint witness population."""
...
class ConflictingCurrentCheckpointWitnessCorpusSnapshot:
    """One canonical payload preserving the conflicting current witness population."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise ConflictAdjudicationError(
                "current checkpoint adjudication corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise ConflictAdjudicationError(
                "current checkpoint witness predecessor differs from 1.17.0"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise ConflictAdjudicationError(
                "current checkpoint witness content order differs from 1.17.0"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise ConflictAdjudicationError(
                "current checkpoint witness content IDs must be unique"
            )
        if not self.witness_attestation_refs:
            raise ConflictAdjudicationError(
                "current checkpoint adjudication requires witness attestations"
            )
        if len(self.witness_attestation_refs) != len(
            set(self.witness_attestation_refs)
        ):
            raise ConflictAdjudicationError(
                "current checkpoint witness attestation refs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise ConflictAdjudicationError(
                "current checkpoint adjudication may not precede 1.17.0"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise ConflictAdjudicationError(
                "current checkpoint adjudication corpus hash differs from payload"
            )
    ...
    def from_document(
        ...
        *,
        ...
        if unknown:
            raise ConflictAdjudicationError(
                "current checkpoint adjudication corpus contains unsupported fields: "
                + ", ".join(unknown)
            )
        ...
        if not isinstance(refs, list):
            raise ConflictAdjudicationError(
                "current checkpoint witness attestation refs must be an array"
            )
        ...
        return cls(
            ...
            witness_attestation_refs=tuple(
                StoredArtifactRef.from_document(
                    _mapping(item, "current checkpoint witness attestation ref")
                )
                for item in refs
            ),
...
@dataclass(frozen=True, slots=True)
class AdjudicationBoundCurrentCheckpointWitnessCorpusSnapshot:
    """The exact conflicting population plus its accepted adjudication authority."""

    corpus: ConflictingCurrentCheckpointWitnessCorpusSnapshot
    ...
    def __post_init__(self) -> None:
        if self.predecessor_corpus_ref != self.witness_predecessor.reference():
            raise ConflictAdjudicationError(
                "adjudication predecessor differs from exact 1.18.0 corpus"
            )
        if self.content_ids != self.witness_predecessor.content_ids:
            raise ConflictAdjudicationError(
                "adjudication content order differs from 1.18.0"
            )
        if (
            ...
            raise ConflictAdjudicationError(
                "adjudication changed the 1.17.0 checkpoint predecessor"
            )
        if (
        ...
        if _parse_timestamp(self.corpus.created_at, "created_at") < _parse_timestamp(
            ...
            raise ConflictAdjudicationError(
                "adjudication successor may not precede 1.18.0"
            )
    ...
    def from_document(
        ...
        *,
        ...
        witness_corpus = ConflictingCurrentCheckpointWitnessCorpusSnapshot.from_document(
            document,
...
def load_current_checkpoint_conflict_adjudication_evidence(
    ...
    *,
    ...
    """Load and reverify the complete 1.19.0 adjudication graph."""
    ...
    if stored_adjudication.reference() != corpus.adjudication_ref:
        raise ArtifactIntegrityError("stored current adjudication differs from corpus")
...
def validate_current_checkpoint_conflict_adjudication(
    *,
    plan: ExperimentPlan,
    corpus: AdjudicationBoundCurrentCheckpointWitnessCorpusSnapshot,
    witness_registry: CheckpointWitnessRegistrySnapshot,
    ...
    """Validate exact resolution while preserving the original witness abstention."""
...
def persist_current_checkpoint_adjudication_bound_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: AdjudicationBoundCurrentCheckpointWitnessCorpusSnapshot,
    ...
    """Append dependencies, then publish the compact 1.19.0 manifest last."""
    ...
    for artifact in (
        ...
        *(item.artifact() for item in witness_attestations),
        ...
        if stored.artifact_hash != artifact.artifact_hash:
            raise ArtifactIntegrityError("stored current conflict graph differs")
    manifest_ref = store.append(corpus.artifact())
    if manifest_ref.artifact_hash != corpus.reference().artifact_hash:
        raise ArtifactIntegrityError(
            "stored current adjudication corpus reference differs"
        )
    return load_current_checkpoint_conflict_adjudication_evidence(
        store,
        corpus=corpus,
        witness_registry=witness_registry,
        witness_policy=witness_policy,
