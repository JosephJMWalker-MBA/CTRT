"""Checkpoint the exact 1.21.0 conflict-adjudicator revocation ledger head."""
...
class CheckpointBoundCurrentCheckpointWitnessConflictAdjudicatorCredentialRevocationCorpusSnapshot:
    """Compact successor binding the exact 1.21.0 revocation-ledger head."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current conflict-adjudicator revocation checkpoint corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current conflict-adjudicator checkpoint predecessor differs from corpus"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current conflict-adjudicator checkpoint content order differs"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current conflict-adjudicator checkpoint content IDs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint successor may not precede its 1.21.0 predecessor"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current conflict-adjudicator checkpoint hash differs from payload"
            )
    ...
    def from_document(
        ...
        *,
        ...
        if unknown:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current conflict-adjudicator checkpoint corpus contains unsupported "
                "fields: " + ", ".join(unknown)
            )
        content_ids = document.get("content_ids")
        if not isinstance(content_ids, list):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current conflict-adjudicator checkpoint content_ids must be an array"
            )
...
def load_current_checkpoint_witness_conflict_adjudicator_credential_revocation_checkpoint_evidence(
    ...
    *,
    ...
    """Load the generic checkpoint graph through the 1.22.0 manifest."""
...
def validate_current_checkpoint_witness_conflict_adjudicator_credential_revocation_checkpoints(
    *,
    plan: ExperimentPlan,
    corpus: (
        CheckpointBoundCurrentCheckpointWitnessConflictAdjudicatorCredentialRevocationCorpusSnapshot
    ...
    """Verify exact 1.21.0 ledger coverage before revocation evaluation."""
...
def persist_current_checkpoint_witness_conflict_adjudicator_revocation_checkpoint_corpus(
    ...
    *,
    ...
    """Append policy, checkpoints, log, then publish the 1.22.0 manifest last."""
