"""Bind named observations to the exact current adjudicator revocation checkpoint."""
...
class WitnessBoundCheckpointWitnessConflictAdjudicatorCredentialRevocationCorpusSnapshot:
    """Compact successor binding named observations to the immutable 1.17.0 head."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise AdjudicatorCheckpointWitnessError(
                "current checkpoint witness corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCheckpointWitnessError(
                "witness predecessor differs from the exact 1.17.0 corpus"
            )
        ...
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise AdjudicatorCheckpointWitnessError(
                "current witness successor may not precede its 1.17.0 predecessor"
            )
...
def load_current_checkpoint_witness_evidence(
    ...
    *,
    ...
    """Load the generic named-witness graph through the 1.18.0 manifest."""
...
def validate_current_checkpoint_witness_attestations(
    *,
    plan: ExperimentPlan,
    corpus: WitnessBoundCheckpointWitnessConflictAdjudicatorCredentialRevocationCorpusSnapshot,
    registry: CheckpointWitnessRegistrySnapshot,
...
def persist_current_checkpoint_witness_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: WitnessBoundCheckpointWitnessConflictAdjudicatorCredentialRevocationCorpusSnapshot,
    ...
    """Append registry, policy, attestations, then publish 1.18.0 last."""
