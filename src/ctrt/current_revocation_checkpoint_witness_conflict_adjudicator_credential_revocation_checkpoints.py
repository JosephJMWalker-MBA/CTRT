"""Checkpoint the exact `1.26.0` revocation-ledger head."""
...
class CheckpointCorpusSnapshot:
    """Compact `1.27.0` successor over the immutable `1.26.0` ledger."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise AdjudicatorCredentialRevocationCheckpointError(
                "current revocation conflict checkpoint corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint predecessor differs from exact 1.26.0 corpus"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint content order differs from 1.26.0"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint content IDs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint successor may not precede 1.26.0"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint corpus hash differs from payload"
            )
    ...
    def from_document(
        ...
        *,
        ...
        if unknown:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "checkpoint corpus contains unsupported fields: "
                + ", ".join(unknown)
            )
...
def _validate_current(
    *,
    ...
    """Verify exact `1.26.0` ledger coverage before revocation evaluation."""
...
def _persist(
    ...
    *,
    ...
    """Append policy, checkpoint, log, then publish `1.27.0` last."""
...
globals()[_LONG_CLASS] = CheckpointCorpusSnapshot
globals()[_LONG_LOAD] = _load
globals()[_LONG_PERSIST] = _persist
globals()[_LONG_VALIDATE] = _validate_current
