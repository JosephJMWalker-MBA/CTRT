"""Close the exact `1.31.0` revocation-ledger head with one checkpoint."""

from __future__ import annotations

import hashlib
from collections.abc import Mapping
from dataclasses import dataclass
from datetime import datetime
from importlib import import_module
from typing import Any, Self, cast

from ctrt.adjudicator_credential_revocation_checkpoints import (
    AdjudicatorCredentialRevocationCheckpointError,
    AdjudicatorCredentialRevocationCheckpointLogSnapshot,
    AdjudicatorCredentialRevocationCheckpointVerificationReport,
    AdjudicatorCredentialRevocationLedgerCheckpointSnapshot,
    AdjudicatorRevocationCheckpointPolicyLifecycle,
    StoredAdjudicatorCredentialRevocationCheckpointEvidence,
)
from ctrt.adjudicator_credential_revocation_checkpoints import (
    load_adjudicator_credential_revocation_checkpoint_evidence as _load_evidence,
)
from ctrt.adjudicator_credential_revocation_checkpoints import (
    persist_checkpoint_bound_adjudicator_revocation_corpus as _persist_corpus,
)
from ctrt.adjudicator_credential_revocation_checkpoints import (
    validate_adjudicator_credential_revocation_checkpoints as _validate,
)
from ctrt.adjudicator_credential_revocation_ledger import (
    AdjudicatorCredentialRevocationLedgerSnapshot,
)
from ctrt.artifact_store import FileSystemArtifactStore, StoredArtifactRef
from ctrt.experiments import ExperimentPlan, VersionedArtifactRef
from ctrt.serialization import CanonicalArtifact, canonical_json_bytes

_predecessor_module = import_module(
    "ctrt.current_revocation_conflict_adjudicator_checkpoint_witness_"
    "conflict_adjudicator_credential_revocation_ledger"
)
RevocationCorpus = vars(_predecessor_module)[
    "RevocationBoundCurrentRevocationConflictAdjudicatorCheckpointWitness"
    "ConflictAdjudicatorCredentialCorpusSnapshot"
]

_PREFIX = (
    "current_revocation_conflict_adjudicator_checkpoint_witness_"
    "conflict_adjudicator_credential_revocation_closure_checkpoint"
)
_LONG_POLICY = (
    "CurrentRevocationConflictAdjudicatorCheckpointWitnessConflictAdjudicator"
    "CredentialRevocationClosureCheckpointPolicySnapshot"
)
_LONG_CORPUS = (
    "ClosureCheckpointBoundCurrentRevocationConflictAdjudicatorCheckpoint"
    "WitnessConflictAdjudicatorCredentialRevocationCorpusSnapshot"
)
_LONG_LOAD = (
    "load_current_revocation_conflict_adjudicator_checkpoint_witness_conflict_"
    "adjudicator_credential_revocation_closure_checkpoint_evidence"
)
_LONG_PERSIST = (
    "persist_current_revocation_conflict_adjudicator_checkpoint_witness_"
    "conflict_adjudicator_credential_revocation_closure_checkpoint_corpus"
)
_LONG_VALIDATE = (
    "validate_current_revocation_conflict_adjudicator_checkpoint_witness_"
    "conflict_adjudicator_credential_revocation_closure_checkpoints"
)


def _mapping(value: object, field_name: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise AdjudicatorCredentialRevocationCheckpointError(
            f"{field_name} must be an object"
        )
    if any(not isinstance(key, str) for key in value):
        raise AdjudicatorCredentialRevocationCheckpointError(
            f"{field_name} keys must be strings"
        )
    return value


def _string(value: object, field_name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise AdjudicatorCredentialRevocationCheckpointError(
            f"{field_name} must be a non-empty string"
        )
    return value


def _boolean(value: object, field_name: str) -> bool:
    if not isinstance(value, bool):
        raise AdjudicatorCredentialRevocationCheckpointError(
            f"{field_name} must be a boolean"
        )
    return value


def _parse_timestamp(value: str, field_name: str) -> datetime:
    normalized = value[:-1] + "+00:00" if value.endswith("Z") else value
    try:
        parsed = datetime.fromisoformat(normalized)
    except ValueError as exc:
        raise AdjudicatorCredentialRevocationCheckpointError(
            f"{field_name} must be an ISO-8601 timestamp"
        ) from exc
    if parsed.tzinfo is None:
        raise AdjudicatorCredentialRevocationCheckpointError(
            f"{field_name} must include a timezone"
        )
    return parsed


def _versioned_ref(value: object, field_name: str) -> VersionedArtifactRef:
    document = _mapping(value, field_name)
    return VersionedArtifactRef(
        artifact_id=_string(
            document.get("artifact_id"),
            f"{field_name}.artifact_id",
        ),
        artifact_version=_string(
            document.get("artifact_version"),
            f"{field_name}.artifact_version",
        ),
        artifact_hash=_string(
            document.get("artifact_hash"),
            f"{field_name}.artifact_hash",
        ),
    )


@dataclass(frozen=True, slots=True)
class ClosureCheckpointPolicySnapshot:
    """Checkpoint rules plus the finite closure condition for this branch."""

    policy_id: str
    policy_version: str
    status: AdjudicatorRevocationCheckpointPolicyLifecycle
    require_exact_event_order: bool
    require_prefix_extension: bool
    require_contiguous_sequence: bool
    require_monotonic_publication_time: bool
    protected_predecessor_ref: VersionedArtifactRef
    branch_state: str
    automatic_successor_layers_allowed: bool
    reopen_requires_documented_failure: bool
    permitted_reopen_trigger: str
    created_at: str
    canonical_payload: bytes
    artifact_hash: str

    def __post_init__(self) -> None:
        _string(self.policy_id, "policy_id")
        _string(self.policy_version, "policy_version")
        _parse_timestamp(self.created_at, "created_at")
        if self.status is not AdjudicatorRevocationCheckpointPolicyLifecycle.ACCEPTED:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint policy must be accepted"
            )
        if not all(
            (
                self.require_exact_event_order,
                self.require_prefix_extension,
                self.require_contiguous_sequence,
                self.require_monotonic_publication_time,
            )
        ):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint requires exact ordered contiguous publication"
            )
        if self.branch_state != "closed":
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint branch_state must be closed"
            )
        if self.automatic_successor_layers_allowed:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint forbids automatic successor layers"
            )
        if not self.reopen_requires_documented_failure:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint requires a documented failure to reopen"
            )
        if self.permitted_reopen_trigger != "concrete-unrepresented-failure":
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint permits only a concrete unrepresented failure"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint policy hash differs from payload"
            )

    @classmethod
    def from_document(
        cls,
        document: Mapping[str, object],
    ) -> Self:
        allowed = {
            "policy_id",
            "policy_version",
            "status",
            "require_exact_event_order",
            "require_prefix_extension",
            "require_contiguous_sequence",
            "require_monotonic_publication_time",
            "protected_predecessor_ref",
            "branch_state",
            "automatic_successor_layers_allowed",
            "reopen_requires_documented_failure",
            "permitted_reopen_trigger",
            "created_at",
        }
        unknown = sorted(set(document) - allowed)
        if unknown:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint policy contains unsupported fields: "
                + ", ".join(unknown)
            )
        payload = canonical_json_bytes(document)
        return cls(
            policy_id=_string(document.get("policy_id"), "policy_id"),
            policy_version=_string(
                document.get("policy_version"),
                "policy_version",
            ),
            status=AdjudicatorRevocationCheckpointPolicyLifecycle(
                _string(document.get("status"), "status")
            ),
            require_exact_event_order=_boolean(
                document.get("require_exact_event_order"),
                "require_exact_event_order",
            ),
            require_prefix_extension=_boolean(
                document.get("require_prefix_extension"),
                "require_prefix_extension",
            ),
            require_contiguous_sequence=_boolean(
                document.get("require_contiguous_sequence"),
                "require_contiguous_sequence",
            ),
            require_monotonic_publication_time=_boolean(
                document.get("require_monotonic_publication_time"),
                "require_monotonic_publication_time",
            ),
            protected_predecessor_ref=_versioned_ref(
                document.get("protected_predecessor_ref"),
                "protected_predecessor_ref",
            ),
            branch_state=_string(document.get("branch_state"), "branch_state"),
            automatic_successor_layers_allowed=_boolean(
                document.get("automatic_successor_layers_allowed"),
                "automatic_successor_layers_allowed",
            ),
            reopen_requires_documented_failure=_boolean(
                document.get("reopen_requires_documented_failure"),
                "reopen_requires_documented_failure",
            ),
            permitted_reopen_trigger=_string(
                document.get("permitted_reopen_trigger"),
                "permitted_reopen_trigger",
            ),
            created_at=_string(document.get("created_at"), "created_at"),
            canonical_payload=payload,
            artifact_hash=f"sha256:{hashlib.sha256(payload).hexdigest()}",
        )

    def reference(self) -> VersionedArtifactRef:
        return VersionedArtifactRef(
            artifact_id=self.policy_id,
            artifact_version=self.policy_version,
            artifact_hash=self.artifact_hash,
        )

    def artifact(self) -> CanonicalArtifact:
        return CanonicalArtifact(
            artifact_id=self.policy_id,
            payload=self.canonical_payload,
            artifact_hash=self.artifact_hash,
        )


@dataclass(frozen=True, slots=True)
class ClosureCheckpointCorpusSnapshot:
    """Compact `1.32.0` successor closing the immutable `1.31.0` head."""

    corpus: Any
    corpus_id: str
    corpus_version: str
    status: str
    declared_content_ids: tuple[str, ...]
    predecessor_corpus_ref: VersionedArtifactRef
    checkpoint_policy_ref: VersionedArtifactRef
    checkpoint_log_ref: VersionedArtifactRef
    checkpoint_head_ref: StoredArtifactRef
    created_at: str
    canonical_payload: bytes
    artifact_hash: str

    def __post_init__(self) -> None:
        for value, field_name in (
            (self.corpus_id, "corpus_id"),
            (self.corpus_version, "corpus_version"),
            (self.status, "status"),
        ):
            _string(value, field_name)
        if self.status != "frozen":
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint predecessor differs from exact 1.31.0"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint content order differs from 1.31.0"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint content IDs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            self.corpus.created_at,
            "predecessor.created_at",
        ):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint successor may not precede 1.31.0"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint corpus hash differs from payload"
            )

    @classmethod
    def from_document(
        cls,
        document: Mapping[str, object],
        *,
        predecessor: Any,
    ) -> Self:
        allowed = {
            "corpus_id",
            "corpus_version",
            "status",
            "content_ids",
            f"{_PREFIX}_predecessor_corpus_ref",
            f"{_PREFIX}_policy_ref",
            f"{_PREFIX}_log_ref",
            f"{_PREFIX}_head_ref",
            "created_at",
        }
        unknown = sorted(set(document) - allowed)
        if unknown:
            raise AdjudicatorCredentialRevocationCheckpointError(
                "closure checkpoint corpus contains unsupported fields: "
                + ", ".join(unknown)
            )
        content_ids = document.get("content_ids")
        if not isinstance(content_ids, list):
            raise AdjudicatorCredentialRevocationCheckpointError(
                "content_ids must be an array"
            )
        payload = canonical_json_bytes(document)
        return cls(
            corpus=predecessor,
            corpus_id=_string(document.get("corpus_id"), "corpus_id"),
            corpus_version=_string(
                document.get("corpus_version"),
                "corpus_version",
            ),
            status=_string(document.get("status"), "status"),
            declared_content_ids=tuple(
                _string(value, "content_ids") for value in content_ids
            ),
            predecessor_corpus_ref=_versioned_ref(
                document.get(f"{_PREFIX}_predecessor_corpus_ref"),
                f"{_PREFIX}_predecessor_corpus_ref",
            ),
            checkpoint_policy_ref=_versioned_ref(
                document.get(f"{_PREFIX}_policy_ref"),
                f"{_PREFIX}_policy_ref",
            ),
            checkpoint_log_ref=_versioned_ref(
                document.get(f"{_PREFIX}_log_ref"),
                f"{_PREFIX}_log_ref",
            ),
            checkpoint_head_ref=StoredArtifactRef.from_document(
                _mapping(
                    document.get(f"{_PREFIX}_head_ref"),
                    f"{_PREFIX}_head_ref",
                )
            ),
            created_at=_string(document.get("created_at"), "created_at"),
            canonical_payload=payload,
            artifact_hash=f"sha256:{hashlib.sha256(payload).hexdigest()}",
        )

    @property
    def content_ids(self) -> tuple[str, ...]:
        return self.declared_content_ids

    def reference(self) -> VersionedArtifactRef:
        return VersionedArtifactRef(
            artifact_id=self.corpus_id,
            artifact_version=self.corpus_version,
            artifact_hash=self.artifact_hash,
        )

    def artifact(self) -> CanonicalArtifact:
        return CanonicalArtifact(
            artifact_id=self.corpus_id,
            payload=self.canonical_payload,
            artifact_hash=self.artifact_hash,
        )


def _require_closure(
    *,
    corpus: ClosureCheckpointCorpusSnapshot,
    policy: ClosureCheckpointPolicySnapshot,
) -> None:
    if policy.protected_predecessor_ref != corpus.predecessor_corpus_ref:
        raise AdjudicatorCredentialRevocationCheckpointError(
            "closure policy protects a different predecessor"
        )
    if corpus.checkpoint_policy_ref != policy.reference():
        raise AdjudicatorCredentialRevocationCheckpointError(
            "closure policy reference differs from corpus"
        )


def load_closure_checkpoint_evidence(
    store: FileSystemArtifactStore,
    *,
    corpus: ClosureCheckpointCorpusSnapshot,
    policy: ClosureCheckpointPolicySnapshot,
    log: AdjudicatorCredentialRevocationCheckpointLogSnapshot,
) -> StoredAdjudicatorCredentialRevocationCheckpointEvidence:
    """Load the complete closure checkpoint graph through `1.32.0`."""

    _require_closure(corpus=corpus, policy=policy)
    return _load_evidence(
        store,
        corpus=cast(Any, corpus),
        policy=cast(Any, policy),
        log=log,
    )


def validate_closure_checkpoints(
    *,
    plan: ExperimentPlan,
    corpus: ClosureCheckpointCorpusSnapshot,
    policy: ClosureCheckpointPolicySnapshot,
    log: AdjudicatorCredentialRevocationCheckpointLogSnapshot,
    ledger: AdjudicatorCredentialRevocationLedgerSnapshot,
    checkpoints: tuple[
        AdjudicatorCredentialRevocationLedgerCheckpointSnapshot, ...
    ],
    verified_at: str,
    revocation_evaluated_at: str,
) -> AdjudicatorCredentialRevocationCheckpointVerificationReport:
    """Verify the exact `1.31.0` head before its revocation evaluation."""

    _require_closure(corpus=corpus, policy=policy)
    report = _validate(
        plan=plan,
        corpus=cast(Any, corpus),
        policy=cast(Any, policy),
        log=log,
        ledger=ledger,
        checkpoints=checkpoints,
        verified_at=verified_at,
    )
    if _parse_timestamp(verified_at, "verified_at") > _parse_timestamp(
        revocation_evaluated_at,
        "revocation_evaluated_at",
    ):
        raise AdjudicatorCredentialRevocationCheckpointError(
            "closure checkpoint must be verified no later than revocation evaluation"
        )
    return report


def persist_closure_checkpoint_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: ClosureCheckpointCorpusSnapshot,
    predecessor_corpus: Any,
    policy: ClosureCheckpointPolicySnapshot,
    log: AdjudicatorCredentialRevocationCheckpointLogSnapshot,
    ledger: AdjudicatorCredentialRevocationLedgerSnapshot,
    checkpoints: tuple[
        AdjudicatorCredentialRevocationLedgerCheckpointSnapshot, ...
    ],
    verified_at: str,
) -> StoredAdjudicatorCredentialRevocationCheckpointEvidence:
    """Append closure policy, checkpoint, log, then publish `1.32.0` last."""

    _require_closure(corpus=corpus, policy=policy)
    return _persist_corpus(
        store,
        plan=plan,
        corpus=cast(Any, corpus),
        predecessor_corpus=cast(Any, predecessor_corpus),
        policy=cast(Any, policy),
        log=log,
        ledger=ledger,
        checkpoints=checkpoints,
        verified_at=verified_at,
    )


globals()[_LONG_POLICY] = ClosureCheckpointPolicySnapshot
globals()[_LONG_CORPUS] = ClosureCheckpointCorpusSnapshot
globals()[_LONG_LOAD] = load_closure_checkpoint_evidence
globals()[_LONG_PERSIST] = persist_closure_checkpoint_corpus
globals()[_LONG_VALIDATE] = validate_closure_checkpoints

__all__ = (
    "AdjudicatorCredentialRevocationCheckpointError",
    "AdjudicatorCredentialRevocationCheckpointLogSnapshot",
    "AdjudicatorCredentialRevocationCheckpointVerificationReport",
    "AdjudicatorCredentialRevocationLedgerCheckpointSnapshot",
    _LONG_POLICY,
    _LONG_CORPUS,
    "StoredAdjudicatorCredentialRevocationCheckpointEvidence",
    _LONG_LOAD,
    _LONG_PERSIST,
    _LONG_VALIDATE,
)
