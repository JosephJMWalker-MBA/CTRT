"""Bind the current revocation-checkpoint conflict credential to status history."""
...
class RevocationBoundCurrentRevocationCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot:
    """Compact `1.26.0` successor over the immutable `1.25.0` credential graph."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator revocation corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator predecessor differs"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator content order differs"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator content IDs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator successor may not precede 1.25.0"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator corpus hash differs"
            )
    ...
    def from_document(
        ...
        *,
        ...
        if unknown:
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator corpus contains "
                f"unsupported fields: {', '.join(unknown)}"
            )
        content_ids = document.get("content_ids")
        if not isinstance(content_ids, list):
            raise AdjudicatorCredentialRevocationError(
                "current revocation conflict-adjudicator content_ids must be an array"
            )
...
def load_current_revocation_checkpoint_witness_conflict_adjudicator_credential_revocation_evidence(
    ...
    *,
    ...
    """Load the generic revocation graph through the `1.26.0` manifest."""
...
def _persist_current_revocation_credential_corpus(
    ...
    *,
    ...
    """Append policy, events, ledger, then publish the `1.26.0` manifest last."""
