"""Bind the current conflict-adjudicator credential to append-only status history."""
...
_PREFIX = "current_checkpoint_witness_conflict_adjudicator_credential_revocation"
...
class RevocationBoundCurrentCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot:
    """Compact `1.21.0` successor over the immutable `1.20.0` credential graph."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation predecessor differs"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation content order differs"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation content IDs must be unique"
            )
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation successor may not precede 1.20.0"
            )
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation corpus hash differs"
            )
    ...
    def from_document(
        ...
        *,
        ...
        if unknown:
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation corpus contains "
                f"unsupported fields: {', '.join(unknown)}"
            )
        content_ids = document.get("content_ids")
        if not isinstance(content_ids, list):
            raise AdjudicatorCredentialRevocationError(
                "current conflict-adjudicator revocation content_ids must be an array"
            )
...
def load_current_checkpoint_witness_conflict_adjudicator_credential_revocation_evidence(
    ...
    *,
    ...
    """Load the generic revocation graph through the `1.21.0` manifest."""
...
def validate_current_checkpoint_witness_conflict_adjudicator_credential_revocation_ledger(
    *,
    plan: ExperimentPlan,
    corpus: RevocationBoundCurrentCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot,
...
def persist_current_checkpoint_witness_conflict_adjudicator_credential_revocation_bound_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: RevocationBoundCurrentCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot,
    predecessor_corpus: CredentialBoundCurrentCheckpointWitnessConflictCorpusSnapshot,
    adjudicator_registry: WitnessConflictAdjudicatorRegistrySnapshot,
    issuer_registry: CredentialIssuerRegistrySnapshot,
    credential_policy: AdjudicatorCredentialPolicySnapshot,
    revocation_policy: AdjudicatorCredentialRevocationPolicySnapshot,
    ...
    """Append policy, events, ledger, then publish the `1.21.0` manifest last."""
