"""Bind named observations to the exact 1.22.0 revocation checkpoint head."""
...
class WitnessBoundCurrentConflictAdjudicatorRevocationCheckpointCorpusSnapshot:
    """Compact successor binding named observations to the immutable 1.22.0 head."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCheckpointWitnessError(
                "witness predecessor differs from exact 1.22.0 corpus"
            )
        ...
        if _parse_timestamp(self.created_at, "created_at") < _parse_timestamp(
            ...
            raise AdjudicatorCheckpointWitnessError(
                "current witness successor may not precede 1.22.0 predecessor"
            )
...
def load_current_conflict_adjudicator_revocation_checkpoint_witness_evidence(
    store: FileSystemArtifactStore,
    *,
    corpus: WitnessBoundCurrentConflictAdjudicatorRevocationCheckpointCorpusSnapshot,
    registry: CheckpointWitnessRegistrySnapshot,
    policy: CheckpointWitnessPolicySnapshot,
) -> StoredAdjudicatorCheckpointWitnessEvidence:
    """Load the named-witness graph through the 1.23.0 manifest."""
...
def validate_current_conflict_adjudicator_revocation_checkpoint_witnesses(
    *,
    plan: ExperimentPlan,
    corpus: WitnessBoundCurrentConflictAdjudicatorRevocationCheckpointCorpusSnapshot,
    registry: CheckpointWitnessRegistrySnapshot,
...
def persist_current_conflict_adjudicator_revocation_checkpoint_witness_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: WitnessBoundCurrentConflictAdjudicatorRevocationCheckpointCorpusSnapshot,
    ...
    """Append registry, policy, attestations, then publish 1.23.0 last."""
