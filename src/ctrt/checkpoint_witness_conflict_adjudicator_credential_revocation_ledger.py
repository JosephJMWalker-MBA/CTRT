"""Bind the checkpoint-fork adjudicator credential to append-only status history."""
...
class RevocationBoundCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot:
    """Compact 1.16.0 successor over the immutable 1.15.0 credential graph."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status != "frozen":
            raise AdjudicatorCredentialRevocationError(
                "checkpoint-fork adjudicator revocation corpus must be frozen"
            )
        if self.predecessor_corpus_ref != self.corpus.reference():
            raise AdjudicatorCredentialRevocationError(
                "checkpoint-fork adjudicator revocation predecessor differs"
            )
        if self.declared_content_ids != self.corpus.content_ids:
            raise AdjudicatorCredentialRevocationError(
                "checkpoint-fork adjudicator revocation content order differs"
            )
        if len(self.declared_content_ids) != len(set(self.declared_content_ids)):
            raise AdjudicatorCredentialRevocationError(
                "checkpoint-fork adjudicator revocation content IDs must be unique"
            )
        _parse_timestamp(self.created_at, "created_at")
        expected = f"sha256:{hashlib.sha256(self.canonical_payload).hexdigest()}"
        if self.artifact_hash != expected:
            raise AdjudicatorCredentialRevocationError(
                "checkpoint-fork adjudicator revocation corpus hash differs"
            )
    ...
    def from_document(
        ...
        *,
        predecessor: CredentialBoundCheckpointWitnessConflictCorpusSnapshot,
    ) -> RevocationBoundCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot:
        allowed = {
            "corpus_id",
            "corpus_version",
        ...
        if unknown:
            raise AdjudicatorCredentialRevocationError(
                "checkpoint-fork adjudicator revocation corpus contains "
                f"unsupported fields: {', '.join(unknown)}"
            )
        content_ids = document.get("content_ids")
        if not isinstance(content_ids, list):
            raise AdjudicatorCredentialRevocationError(
                "checkpoint-fork adjudicator revocation content_ids must be an array"
            )
...
def load_checkpoint_witness_conflict_adjudicator_credential_revocation_evidence(
    ...
    *,
    ...
    """Load the generic revocation graph through the 1.16.0 manifest."""
...
def validate_checkpoint_witness_conflict_adjudicator_credential_revocation_ledger(
    *,
    plan: ExperimentPlan,
    corpus: RevocationBoundCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot,
...
def persist_checkpoint_witness_conflict_adjudicator_credential_revocation_bound_corpus(
    store: FileSystemArtifactStore,
    *,
    plan: ExperimentPlan,
    corpus: RevocationBoundCheckpointWitnessConflictAdjudicatorCredentialCorpusSnapshot,
    predecessor_corpus: CredentialBoundCheckpointWitnessConflictCorpusSnapshot,
    adjudicator_registry: WitnessConflictAdjudicatorRegistrySnapshot,
    issuer_registry: CredentialIssuerRegistrySnapshot,
    credential_policy: AdjudicatorCredentialPolicySnapshot,
    revocation_policy: AdjudicatorCredentialRevocationPolicySnapshot,
    ...
    """Append policy, events, ledger, then publish the 1.16.0 manifest last."""
