"""Resolve the exact current checkpoint witness conflict before executing PR #40."""
...
class AdjudicatedCurrentCheckpointWitnessRunnerStage(StrEnum):
...
class AdjudicatedCurrentCheckpointWitnessRunnerStatus(StrEnum):
    """A receipt exists only after complete storage reverification."""

    VERIFIED = "verified"


class AdjudicatedCurrentCheckpointWitnessExperimentError(RuntimeError):
    """Fail-closed error preserving stage and completed content IDs."""

    def __init__(
        self,
        stage: AdjudicatedCurrentCheckpointWitnessRunnerStage,
        message: str,
        *,
...
ADJUDICATED_CURRENT_CHECKPOINT_WITNESS_VERIFIED_CHECKS = (
    "exact-1.18.0-current-witness-predecessor-preserved",
    "exact-conflicting-current-witness-population-bound",
    "original-current-witness-abstention-preserved",
    "exact-current-witness-conflict-adjudicator-registry-bound",
    "exact-current-witness-conflict-adjudication-policy-bound",
    "current-witness-fork-evidence-reverified",
    "current-witness-dissent-preserved",
    "resolved-head-restricted-to-exact-1.17.0-checkpoint-head",
    "current-witness-adjudication-and-downstream-outcomes-finalized-separately",
)
...
class AdjudicatedCurrentCheckpointWitnessFinalManifest:
    """Final marker preserving conflict, resolution, and every delegated outcome."""

    final_id: str
    experiment_run_id: str
    status: AdjudicatedCurrentCheckpointWitnessRunnerStatus
    conflicting_witness_outcome: CheckpointWitnessDecisionOutcome
    current_resolution_status: WitnessConflictResolutionStatus
    current_conflict_adjudication_outcome: WitnessConflictAdjudicationOutcome
    ...
    def __post_init__(self) -> None:
        ...
        if not self.witness_attestation_refs:
            raise ValueError("adjudicated current witness conflict requires attestations")
        if len(self.witness_attestation_refs) != len(
            set(self.witness_attestation_refs)
        ):
            raise ValueError("adjudicated current witness refs must be unique")
        downstream = (
        ...
        if (
            ...
            if any(item is not None for item in downstream):
                raise ValueError(
                    "current adjudication abstention may not claim downstream outcomes"
                )
            if self.predecessor_witness_final_ref is not None:
                raise ValueError(
                    "current adjudication abstention may not contain PR #40 final"
                )
            ...
            if self.predecessor_witness_final_ref is None:
                raise ValueError("current adjudication execution requires PR #40 final")
            if self.resolved_current_witness_outcome is None:
                raise ValueError(
        ...
        if (
            ...
            raise ValueError("adjudicated current witness final lost verified checks")
        _parse_timestamp(self.completed_at, "completed_at")


@dataclass(frozen=True, slots=True)
class VerifiedAdjudicatedCurrentCheckpointWitnessReceipt:
    """Proof of conflict resolution plus optional exact PR #40 result."""
    ...
    witness_decision_ref: StoredArtifactRef
    adjudication_decision_ref: StoredArtifactRef
    predecessor_witness_receipt: VerifiedCurrentCheckpointWitnessReceipt | None
    ...
    def __post_init__(self) -> None:
        expected_status = AdjudicatedCurrentCheckpointWitnessRunnerStatus.VERIFIED
        if self.status is not expected_status:
            raise ValueError("verified adjudicated current witness status required")
        downstream = (
            self.resolved_current_witness_outcome,
            self.current_revocation_outcome,
            self.current_credential_outcome,
        ...
        if (
            ...
            if self.predecessor_witness_receipt is not None:
                raise ValueError(
                    "current adjudication abstention may not contain PR #40 receipt"
                )
            if any(item is not None for item in downstream):
                raise ValueError(
            ...
            if delegated is None:
                raise ValueError("current adjudication execution requires PR #40 receipt")
            if delegated.experiment_run_id != self.experiment_run_id:
                raise ValueError("PR #40 receipt belongs to another experiment run")
            if (
                delegated.current_checkpoint_witness_outcome
                is not self.resolved_current_witness_outcome
                or delegated.revocation_outcome is not self.current_revocation_outcome
                or delegated.credential_outcome is not self.current_credential_outcome
                or delegated.checkpoint_witness_outcome
                is not self.lower_checkpoint_witness_outcome
                or delegated.resolution_status is not self.lower_resolution_status
                ...
                raise ValueError("PR #40 receipt differs from adjudicated receipt")
            suffix = (
                "completion"
                if self.terminal_outcome is ReviewDecisionOutcome.EXECUTE
                else "terminal-abstention"
        ...
        if (
            ...
            raise ValueError("verified adjudicated current witness lost checks")
        _parse_timestamp(self.completed_at, "completed_at")


class AdjudicatedCurrentCheckpointWitnessExperimentRunner:
    """Adjudicate exact 1.19.0 conflict before executing PR #40."""

    def __init__(self, *, artifact_store: FileSystemArtifactStore) -> None:
        self._store = artifact_store
        self._runner = WitnessGatedCurrentCheckpointExperimentRunner(
    ...
    def _preflight(
        *,
        ...
        if plan.status is not ExperimentPlanStatus.FROZEN:
            raise ValueError("adjudicated current witness conflict requires frozen plan")
        ...
        if corpus.predecessor_corpus_ref != witness_predecessor.reference():
            raise ValueError("adjudication corpus must bind exact 1.18.0 predecessor")
        if corpus.corpus.witness_registry_ref != witness_registry.reference():
            raise ValueError("current witness registry differs from adjudication corpus")
        if corpus.corpus.witness_policy_ref != witness_policy.reference():
            raise ValueError("current witness policy differs from adjudication corpus")
        if corpus.corpus.witness_attestation_refs != tuple(
            item.reference() for item in conflict_witness_attestations
        ):
            raise ValueError("conflicting current witness population differs from corpus")
        if corpus.adjudicator_registry_ref != conflict_adjudicator_registry.reference():
            raise ValueError("current conflict adjudicator registry differs from corpus")
        if corpus.adjudication_policy_ref != conflict_adjudication_policy.reference():
            raise ValueError("current conflict adjudication policy differs from corpus")
        if corpus.adjudication_ref != conflict_adjudication.reference():
            raise ValueError("current conflict adjudication record differs from corpus")
        ...
        if not (
            ...
            raise ValueError(
                "successor, witness conflict, adjudication, and PR #40 chronology differs"
            )
    ...
    def _verify_final(
        self,
        *,
        final: AdjudicatedCurrentCheckpointWitnessFinalManifest,
        ...
        if self._store.get(
            ...
            raise ArtifactIntegrityError("stored adjudicated current witness final differs")
        if self._store.get(
            ...
            raise ArtifactIntegrityError("stored current adjudication corpus differs")
        ...
        if predecessor.payload != witness_predecessor.artifact().payload:
            raise ArtifactIntegrityError("stored 1.18.0 witness predecessor differs")
        if self._store.get(
        ...
        if self._store.get(
            ...
            raise ArtifactIntegrityError("stored current adjudicator registry differs")
        if self._store.get(
            ...
            raise ArtifactIntegrityError("stored current adjudication policy differs")
        if self._store.get(
            ...
            raise ArtifactIntegrityError("stored current adjudication record differs")
    ...
    def run(
        ...
        *,
        ...
        lower_witness_registry: CheckpointWitnessRegistrySnapshot,
        lower_witness_policy: CheckpointWitnessPolicySnapshot,
        lower_conflict_witness_attestations: tuple[
            CheckpointWitnessAttestationSnapshot, ...
        ],
        lower_predecessor_witness_attestations: tuple[
            CheckpointWitnessAttestationSnapshot, ...
        ],
        ...
        inherited_witness_attestations: tuple[
            CheckpointWitnessAttestationSnapshot, ...
        ],
        ...
        """Return adjudication abstention or the exact delegated PR #40 result."""
        ...
        try:
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.PREFLIGHT,
                str(exc),
            ) from exc

        try:
            evidence = load_current_checkpoint_conflict_adjudication_evidence(
                self._store,
                corpus=corpus,
                witness_registry=witness_registry,
                witness_policy=witness_policy,
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.EVIDENCE_LOADING,
                str(exc),
            ) from exc
        ...
        try:
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.WITNESS_VALIDATION,
                str(exc),
            ) from exc
        ...
        try:
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.WITNESS_DECISION_PERSISTENCE,
                str(exc),
            ) from exc

        try:
            adjudication_decision = validate_current_checkpoint_conflict_adjudication(
                plan=plan,
                corpus=corpus,
                witness_registry=witness_registry,
                witness_policy=witness_policy,
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.ADJUDICATION_VALIDATION,
                str(exc),
            ) from exc
        ...
        try:
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.ADJUDICATION_DECISION_PERSISTENCE,
                str(exc),
            ) from exc

        delegated: VerifiedCurrentCheckpointWitnessReceipt | None = None
        if (
            ...
            try:
                delegated = self._runner.run(
                    ...
                    witness_predecessor=lower_witness_predecessor,
                    ...
                    witness_registry=lower_witness_registry,
                    witness_policy=lower_witness_policy,
                    conflict_witness_attestations=lower_conflict_witness_attestations,
                    ...
                    conflict_adjudication_policy=lower_conflict_adjudication_policy,
                    conflict_adjudication=lower_conflict_adjudication,
                    ...
                    inherited_witness_evaluated_at=(
                        inherited_witness_evaluated_at
                    ),
                    ...
                    prior_completed_at=lower_completed_at,
            ...
            except CurrentCheckpointWitnessExperimentError as exc:
                raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                    AdjudicatedCurrentCheckpointWitnessRunnerStage.WITNESS_EXECUTION,
                    str(exc),
        ...
        if delegated is None:
            ...
            current_revocation_outcome = delegated.revocation_outcome
            current_credential_outcome = delegated.credential_outcome
            lower_checkpoint_witness_outcome = delegated.checkpoint_witness_outcome
            lower_resolution_status = delegated.resolution_status
            lower_conflict_adjudication_outcome = (
                delegated.conflict_adjudication_outcome
            )
            lower_predecessor_witness_outcome = delegated.predecessor_witness_outcome
        ...
        final = AdjudicatedCurrentCheckpointWitnessFinalManifest(
            final_id=final_id,
            experiment_run_id=experiment_run_id,
            status=AdjudicatedCurrentCheckpointWitnessRunnerStatus.VERIFIED,
            conflicting_witness_outcome=witness_decision.outcome,
            current_resolution_status=adjudication_decision.resolution_status,
            current_conflict_adjudication_outcome=adjudication_decision.outcome,
            ...
            verified_checks=ADJUDICATED_CURRENT_CHECKPOINT_WITNESS_VERIFIED_CHECKS,
            completed_at=completed_at,
        )
        try:
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.FINAL_PERSISTENCE,
                str(exc),
        ...
        try:
            ...
            raise AdjudicatedCurrentCheckpointWitnessExperimentError(
                AdjudicatedCurrentCheckpointWitnessRunnerStage.VERIFICATION,
                str(exc),
                completed_content_ids=(
                    plan.content_ids
                    if terminal_outcome is ReviewDecisionOutcome.EXECUTE
        ...
        return VerifiedAdjudicatedCurrentCheckpointWitnessReceipt(
            experiment_run_id=experiment_run_id,
            status=AdjudicatedCurrentCheckpointWitnessRunnerStatus.VERIFIED,
            conflicting_witness_outcome=witness_decision.outcome,
            current_resolution_status=adjudication_decision.resolution_status,
